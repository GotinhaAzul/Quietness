...
| 70 | **T11 — Core behaviors (separation + following)** — Created `stores/pet.ts` with cursor coord + typing stores; NoteEditor emits cursor coords on doc change; `+page.svelte` preview container gets `id="preview-panel"`; FlamePet now positions big flame at preview panel bottom-right, lerps small particle toward editor cursor on typing, returns to big flame on idle, hides big flame in edit-only mode | May 23 |
| 71 | **T12 — Idle timer behaviors** — Rewrote `tick()` in `FlamePet.svelte` with 3-state state machine: active (<2s, particle follows cursor), idle-spin (2-10s, particle spins in place via `spSpinning` + faster spark orbit), idle-merged (10s+, particle returns and merges with big flame); big flame randomly triggers burst and wiggle animations during all idle states via per-frame probability checks | May 23 |
| 72 | **T13 — Mode-specific behavior** — Edit-only mode: small flame draws at all times (was hidden at rest), enters from screen edge (-30px off-screen left) with smooth lerp entrance; split/preview mode: particle stays separated on mode switch and visibly lerps back to big flame for graceful return; `drawSmallParticle` guard flipped to `showBigFlame && !isSeparated` so small flame is always visible in edit-only mode; `$effect` tracks mode changes and resets idle timer + particle state | May 23 |
| 73 | **T14 — Pet settings** — Split `PetSettings.enabled` into `bigFlameEnabled` + `smallParticleEnabled` booleans; added Pet tab in SettingsModal with two toggles and 5 color pickers (core/inner/mid/outer/ember); added `PetSettings` + `PetColorPalette` to Rust backend `Settings` struct; changes persist via existing settings save/load cycle | May 23 |
