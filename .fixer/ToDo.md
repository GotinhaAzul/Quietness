...
- [ ] `T24` **Remove duplicate folder loading on startup** — Keep a single startup `loadFolders()` path between `+page.svelte` and `FolderTree.svelte` so the app does not issue two immediate `list_folders` calls. Completion: startup loads folders once, and the folder tree still updates from the shared `folders` store.
- [ ] `T25` **Remove duplicate note-list loading on first render** — In `NoteList.svelte`, rely on one initial load path instead of both `onMount(loadNoteList)` and the immediate `$effect` call. Completion: initial sidebar note loading performs one backend request while still reacting to folder/search/scope changes.
- [ ] `T26` **Avoid full folder-tree rebuild on expand/collapse** — Separate folder hierarchy construction from expanded-state toggling in `FolderTree.svelte`, so UI expand/collapse does not rebuild and re-sort the whole tree. Completion: expanding folders changes only expansion state while folder-list changes still rebuild the hierarchy.
- [ ] `T27` **Remove redundant CodeMirror reconfigure after saved-state restore** — In `NoteEditor.svelte`, avoid the extra `view.dispatch({ effects: ... })` immediately after `view.setState(saved)` when restoring a valid per-note editor state. Completion: switching back to a cached note restores once without an extra CodeMirror transaction.
- [ ] `T28` **Use indexed wikilink lookup in preview clicks** — In `NotePreview.svelte`, derive a lowercase note-name index for both existing-note detection and click navigation instead of rebuilding a `Set` and then doing linear `$notes.find()` on click. Completion: wikilink clicks resolve through O(1) lookup without changing render output or navigation behavior.
- [ ] `T29` **Use native close-token search in wikilink parser** — In `wikilinks.ts`, replace the manual char-by-char scan for `]]` with a native string search while preserving the current invalidation rules for nested `[` and newline. Completion: wikilink parsing keeps identical behavior with less hot-path work during Markdown rendering.
- [ ] `T30` **Cache lowercase sort keys in Rust listings** — Replace repeated `name.to_lowercase()` inside Rust sort comparators with cached sort keys (`sort_by_cached_key` or equivalent) in note, folder, search-result, and theme listings. Completion: sorting keeps the same order with fewer allocations on large vaults.
- [ ] `T31` **Remove redundant filesystem existence checks** — In `fs.rs`, prefer idempotent `create_dir_all` and direct read/fallback flows where safe, instead of `exists()` checks before creating dirs or loading settings. Completion: common startup/settings paths use fewer filesystem syscalls without changing error fallback behavior.
